...
int nDevices = 2000;
int nGateways = 4;
double sideLength = 6400; //Note that due to model updates, 7500 m is no longer the maximum distance
...
main (int argc, char *argv[])
{
  ...
  double yLength = 64;
  double deltaY = 17;
  int gridWidth = 2 * sideLength / (xLength + deltaX);
  int gridHeight = 2 * sideLength / (yLength + deltaY);
  ...
  std::cout << tracker.PrintPhyPacketsPerGw (Seconds (0), appStopTime,0) << std::endl;
  std::cout << tracker.PrintPhyPacketsPerGw (Seconds (0), appStopTime,1) << std::endl;
  std::cout << tracker.PrintPhyPacketsPerGw (Seconds (0), appStopTime,2) << std::endl;
  std::cout << tracker.PrintPhyPacketsPerGw (Seconds (0), appStopTime,3) << std::endl;
  return 0;
}
