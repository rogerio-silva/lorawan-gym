...
main(int argc, char* argv[])
{
    ...
    if (verbose)
    {
        ...
        LogComponentEnable("ns3::LorawanRlMobilityModel", ns3::LOG_LEVEL_ALL);
    ...
    Config::SetDefault("ns3::LorawanRlMobilityModel::Step",
                       DoubleValue(movementStep)); // Time step of env
    Config::SetDefault("ns3::LorawanRlMobilityModel::Reward", DoubleValue(reward)); // Reward
    Config::SetDefault("ns3::LorawanRlMobilityModel::Bounds",
                       BoxValue(Box(0.0, 10000.0, 0.0, 10000.0, 30.0, 30.0)));
    Config::SetDefault("ns3::LorawanRlMobilityModel::SimulationTime", BooleanValue(false));


    //    Config::SetDefault("ns3::LorawanRlMobilityModel::NumberOfDevices", UintegerValue(nDevices));
    //    Config::SetDefault("ns3::LorawanRlMobilityModel::NumberOfGateways", UintegerValue(nGateways));
    /************************
     *  Create the helpers  *
     ************************/
...
//                                        MakeCallback(&LorawanGymEnv::TransmissionCallback));
//    }
    ...
    {
        uavs_positions->Add(Vector(0, 0, 0));
    }

    mobilityGW.SetMobilityModel("ns3::LorawanRlMobilityModel");
     ...
     ************************************/
    Simulator::Schedule(Seconds(0.01), &DoSetInitialPositions);

//     Configura os traces dos gateways
...
//            MakeCallback(&LorawanGymEnv::PacketReceptionCallback));
//            gwPhy->TraceConnectWithoutContext("LostPacketBecauseInterference",
//                                              MakeCallback(&LorawanGymEnv::InterferenceCallback));
//            gwPhy->TraceConnectWithoutContext("LostPacketBecauseNoMoreReceivers",
//                                              MakeCallback(&LorawanGymEnv::NoMoreReceiversCallback));
//            gwPhy->TraceConnectWithoutContext("LostPacketBecauseUnderSensitivity",
//                                              MakeCallback(&LorawanGymEnv::UnderSensitivityCallback));
        }
    ...
    applicationContainer.Start(Seconds(0.5));
...
void DoSetInitialPositions(){
    std::cout << "Initial UAVs deployment at " << Simulator::Now() << std::endl;
...
//         double rk;
//         rk = (size / ((*p).second.receivedTime.GetSeconds() -
//         (*p).second.sentTime.GetSeconds())); deviceSimulatedData[devID][0].push_back(sf);
...
//     }
//
//     std::cout << "Total: " << numPackets << " Sent: " << pkt_transmitted << " Lost: " << _lost
...
//         {
//             // lembre que o ID do gateways é enumerado após todos os devices, logo numero
//             sequencial
//             //  do GW é contado subtraindo o m_number_of_devices. Ainda que os pacotes originados
//             do
//             //  mesmo device no mesmo instante é recebido pelos gateways na área de alcance, <e
//             que
...
//             //      if ((*it).second.outcomeNumber == 1 || (*it).second.receivedTime == Seconds
//             (0)) if ((*it).second.receivedTime == Seconds(0))
//             {
...
// Config::ConnectWithoutContext(oss.str(), MakeBoundCallback(FunctionToDefine, openGymInterface));
