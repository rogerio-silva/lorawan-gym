...
LorawanRl::DoSetPosition(const Vector& position)
{
    NS_LOG_FUNCTION(this);
    m_position = position;
    if(!m_lorawanGymEnv && m_simulation_time){
        CreateGymEnv();
    }
    NotifyCourseChange();
}
...
{
    NS_LOG_FUNCTION(this);
    for (NodeList::Iterator i = NodeList::Begin(); i != NodeList::End() - m_number_of_gateways; ++i)
    {
        m_devices.Add(*i);
    }
}
...
{
    NS_LOG_FUNCTION(this);
    for (NodeList::Iterator i = NodeList::Begin() + m_number_of_devices; i != NodeList::End(); ++i)
    {
        m_gateways.Add(*i);
    }
}
